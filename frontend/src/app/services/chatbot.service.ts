...
export class ChatbotService implements OnInit {
  ...
  private demandeAlimentation = "Inventes 1 conseil sur l'alimentation sous format json comprenant titre nommé nom + adresse nommé adresse + date nommé date sous la forme aaaa-mm-jj, sois inventif, ta réponse ne devra contenir que le JSON elle commencera par le json et terminera par le json";
  private demandeHydratation = "Inventes 1 conseil sur l'hydratation  sous format json comprenant titre nommé nom + adresse nommé adresse + date nommé date sous la forme aaaa-mm-jj, sois inventif, ta réponse ne devra contenir que le JSON elle commencera par le json et terminera par le json";
  ...
  getDemandeInfos(categorieCourante: number) {
    ...
    if (categorieCourante == 0) {
      ...
      return { demande: this.demandeHydratation, num: 4 };
    }
  }
}
